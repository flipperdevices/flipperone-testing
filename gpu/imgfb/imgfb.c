...
uint8_t *decode_farbfeld(FILE *fp, uint32_t *width_, uint32_t *height_) {
	...
	if (pixel_count > SIZE_MAX / 4) {
		eprintf("farbfeld image dimensions too large\n");
		return NULL;
	}
	// calloc() performs its own internal overflow-checked multiplication of its two arguments
	pixels = calloc((size_t)pixel_count, 4);
	if (!pixels) { eprintf("calloc: %s\n", strerr); return NULL; }
	*width_ = width;
	*height_ = height;
	for (uint32_t y = 0; y < height; ++y) {
		for (uint32_t x = 0; x < width; ++x) {
